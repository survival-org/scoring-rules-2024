...
- The **RCLL** and its weighted version **wRCLL** (**RCLL\*** in the paper)
...
```{r}
res = readRDS("results/res_sims10000_distrs1000_1.old.rds") |>
...
res |>
  rename(`RCLL*` = RRCLL, `C-index` = surv.cindex, 
         `D-Calibration` = surv.dcalib, ISBS = surv.graf) |>
  DT::datatable(rownames = FALSE, options = list(searching = FALSE)) |>
...
Our experiment demonstrates substantial divergences in model assessment depending on the chosen metric (especially in the case of the scoring rules which are all theoretically proper).
