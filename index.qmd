...
The theoretical shift (i.e. bias) of the global minimum $x^*$ of the expected SBS from the true survival probability is the survival tail product $\times$ a survival probability ratio term at $\tau^*$ (for small $\varepsilon$):
...
### $\varepsilon$ vs bias
```{r}
#| fig-cap: "Relationship between survival tail product $\\epsilon$ and theoretical shift from the global minimum, using a sample of data from the Weibull simulation experiments"
#| cache: true
#| fig-width: 9
#| fig-height: 5
...
plot_data |> 
  ...
  ggplot(aes(x = epsilon, y = shift, color = factor(n))) +
    ...
    labs(
      x = expression(varepsilon == S[Y](t[max]) %*% S[C](t[max])),
...
### $S_Y(\tau^*)$ vs bias
```{r}
#| fig-cap: "Relationship between true survival $S_Y(\\tau^*)$ and theoretical shift from the global minimum, using a sample of data from the Weibull simulation experiments"
#| cache: true
#| fig-width: 9
#| fig-height: 5
...
### $S_C(\tau^*)$ vs bias
```{r}
#| fig-cap: "Relationship between censoring survival $S_C(\\tau^*)$ and theoretical shift from the global minimum, using a sample of data from the Weibull simulation experiments"
#| cache: true
#| fig-width: 9
#| fig-height: 5
...
```{r}
#| fig-cap: "Interpolated surface of theoretical shift $x^\\star - S_Y(\\tau^*)$ as a function of $S_Y(\\tau^*)$ and $S_C(\\tau^*)$ for $n = 100$. Panels correspond to early, median, and late evaluation times."
#| cache: true
#| fig-width: 10
#| fig-height: 5
...
ggplot(surface_data, aes(x = S_C, y = S_Y, fill = shift)) +
  ...
  theme_bw(base_size = 14) +
  theme(panel.spacing = unit(1, "lines"))
#ggsave(filename = "a.png", dpi = 300, height = 5, width = 10, units = "in")
```
:::
