...
```{r}
#| fig-cap: "Relationship between survival tail product estimate $\\hat\\epsilon = S_Y(t_{max}) \\times S_C(t_{max})$ and theoretical shift from the global minimum, using a sample of data from the Weibull simulation experiments"
#| cache: true
#| fig-width: 9
#| fig-height: 5
...
```{r}
#| fig-cap: "Interpolated surface of theoretical shift $x^\\star - S_Y(\\tau^*)$ as a function of $S_Y(\\tau^*)$ and $S_C(\\tau^*)$ for $n = 50$ (with $\\varepsilon$ ~ 0.02). Panels correspond to early, median, and late evaluation times."
#| cache: true
#| fig-width: 10
#| fig-height: 5
...
```{r}
#| fig-cap: "Interpolated surface of theoretical shift $x^\\star - S_Y(\\tau^*)$ as a function of $S_Y(\\tau^*)$ and $S_C(\\tau^*)$ for $n = 100$ (with $\\varepsilon$ ~ 0.01). Panels correspond to early, median, and late evaluation times."
#| cache: true
#| fig-width: 10
#| fig-height: 5
