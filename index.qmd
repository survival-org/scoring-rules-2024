...
Across all grid sizes, the ordering of the fitted models remains largely unchanged in both censoring scenarios, indicating that interpolation mainly affects the absolute scale of the RCLL rather than its ability to rank models.

- **Interpolation bias affects the true model and fitted models in opposite directions.**
For the true model, coarse prediction grids inflate the interpolated RCLL above the exact likelihood, whereas all fitted models exhibit systematically smaller RCLL values, making them appear artificially closer to the truth.
This bias is largely eliminated using sufficiently dense prediction grids.
...
- **Dense prediction grids are recommended when reporting absolute RCLL values.**
While moderate grid coarsening has little effect on model ranking, sufficiently dense prediction grids are needed to minimize interpolation bias, particularly when accurate absolute RCLL values are of interest.
:::
