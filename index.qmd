...
:::{.callout-note}
This benchmark complements the simulation study by comparing how different evaluation metrics rank survival learners across eight heterogeneous, low-dimensional, real-world datasets.
...
task_info |>
  ...
  DT::formatRound(columns = 6, digits = 2) |>
  DT::formatStyle(
...
Model performance is assessed using three survival scoring rules (RCLL, RCLL*, and ISBS), D-calibration (strictly calibration metric), and Harrell's C-index (strictly discrimination metric).
All models are evaluated using 10 repetitions of 5-fold cross-validation without
...
In each dataset, the learners are ordered by median RCLL, which is the most discriminative metric in this benchmark.
...
We compute Pearson correlations between the resulting ranking vectors and visualize the empirical agreement results as a correlation heatmap.
...
The real-world benchmark confirms that **the choice of evaluation metric can substantially affect the ranking of survival learners**.

Several consistent patterns emerge:

- **RCLL provides the clearest separation between competing models.** Across nearly all datasets, RCLL exhibits differences between models with relatively little overlap between the boxplots.

- **ISBS shows moderate agreement with RCLL.** Among the evaluated metrics, ISBS exhibits the strongest agreement with discrimination ($\rho = 0.63$ with the C-index rankings) while also displaying moderate agreement with RCLL ($\rho = 0.43$).

- **RCLL and RCLL\* often produce different learner rankings.** Although RCLL\* is derived from RCLL, their empirical agreement is relatively weak ($\rho = 0.16$), which is expected given that RCLL\* is not proper.

- **D-calibration behaves largely independently of the other metrics.** Its rank
correlations with all remaining measures are close to zero, indicating that calibration alone captures a different aspect of predictive performance.
