...
Notably, RCLL\*\* correlates with RCLL ($r = 0.63$) and more moderately with ISBS ($r = 0.54$), indicating some underlying coherence.

For example, on the **veteran** and **whas** datasets, which exhibit pronounced non-proportional hazards, the CoxPH model is theoretically misspecified while RSF is more suitable to model non-linear effects.
Here, **RCLL\*\*** appropriately identifies RSF as superior, whereas RCLL and ISBS still prefer CoxPH - demonstrating potential practical limitations of the latter metrics in such settings.

These findings highlight a critical need for further research: not all metrics labeled as "proper" in theory are equally discerning in real applications.
:::
