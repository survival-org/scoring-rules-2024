...
We now evaluate how effectively each scoring rule discriminates between increasingly misspecified models.
For each of the two censoring tasks (low and high), we compare the excess loss, i.e. the difference in empirical loss between a candidate model and the true data-generating process, i.e. $\Delta L = L(\hat S) - L(S_Y)$, against the mean integrated absolute error (MIAE) between the predicted and true survival functions.
See more details in the manuscript regarding the benchmark setup.

The plots below show results for test sets of size $n = 1000$, with panels corresponding to three scoring rules:
...
- **ISBS**: Integrated Survival Brier Score up to the 90th percentile of observed times

In each panel, models are arranged by increasing MIAE, i.e. greater deviation from the true survival distribution (left to right).
...
#| eval: false
# figure 5
p11 = plot_score_diff(res, task_id_val = "low", meas = c("rcll", "sbs", "isbs"),
                     n_test_val = 1000, legend_position = "none")
p22 = plot_score_diff(res, task_id_val = "high", meas = c("rcll", "sbs", "isbs"),
                     n_test_val = 1000, legend_position = "bottom") +
