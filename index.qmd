...
- **RCLL provides the greatest discrimination between competing models.** Across nearly all datasets, RCLL produces the clearest separation between learners, whereas Harrell's C-index often assigns similar scores to models with noticeably different predictive distributions.

- **ISBS exhibits intermediate behaviour.** It generally distinguishes competing models more clearly than the C-index, although less consistently than RCLL, and shows moderate agreement with both RCLL ($\rho = 0.43$) and the C-index ($\rho = 0.64$).
...
Overall, these findings complement the simulation study by demonstrating that the empirical behaviour of survival evaluation metrics differs considerably on real datasets, emphasizing the importance of selecting evaluation metrics that align with the intended prediction objective.
:::
