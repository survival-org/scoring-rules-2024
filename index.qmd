...
plot_rcll_table = function(
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      style = "caption-side: top; text-align: center; font-size:150%",
      sprintf("RCLL (mean \u00B1 SD) for %s censoring task, n = %d.", task, n_test)
    )
  )
}
```
...
```{r}
plot_rcll_table(res_tbl, task = "low", n_test = 50)
```

```{r}
plot_rcll_table(res_tbl, task = "high", n_test = 50)
```
...
```{r}
plot_rcll_table(res_tbl, task = "low", n_test = 100)
```

```{r}
plot_rcll_table(res_tbl, task = "high", n_test = 100)
```
...
```{r}
plot_rcll_table(res_tbl, task = "low", n_test = 250)
```

```{r}
plot_rcll_table(res_tbl, task = "high", n_test = 250)
```
...
```{r}
plot_rcll_table(res_tbl, task = "low", n_test = 500)
```

```{r}
plot_rcll_table(res_tbl, task = "high", n_test = 500)
```
...
- **Prediction grid resolution affects the absolute RCLL values, but not model rankings or empirical properness.**
Across all grid resolutions, the true model consistently achieves the lowest RCLL, with all misspecified models retaining positive excess loss.

- **The RCLL of the true model with linear interpolation of the density is biased upward on coarse grids.**
As the prediction grid becomes denser, the interpolated score converges to the exact log-likelihood, although convergence is slower under heavy censoring.

- **Misspecified models have downward-biased RCLL on coarse grids.**
This makes models appear artificially closer to the true model regardless of their level of misspecification.

- **Dense prediction grids are therefore essential when reporting absolute RCLL values.**
While relative model comparisons remain robust, coarse grids can substantially distort the magnitude of the RCLL through interpolation bias.

- **Model rankings remain largely unchanged.**
Although absolute RCLL values change? on coarse grids, the relative ordering of models is remarkably stable across grid resolutions.
:::
