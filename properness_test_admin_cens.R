#' Testing properness of scoring rules with the presense of administrative censoring
#' Same as `properness_test.R` but with an additional administrative censoring at
#' the 80% percentile of true event times.
#' Execute: `Rscript properness_test_admin_cens.R 10000 1000 10000 FALSE 42`
...
run = function(surv_shape, cens_shape, pred_shape,
  ...
  x = mclapply(seq.int(num_distrs), function(i) {
    ...
    # 80th percentile of true event times
    admin_time = quantile(true_y, 0.8)

    # observed outcomes (time, status/delta) with admin. censoring
    obs_t = pmin(true_y, true_c, admin_time)
    obs_d = true_y <= pmin(true_c, admin_time)
...
# SAVE RESULTS ----
saveRDS(res, file = sprintf("res_sims%s_distrs%s_n%s_%i_admin_cens.rds", num_sims, num_distrs, num_samples, estimate_cens))
